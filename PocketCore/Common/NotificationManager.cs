...
namespace Pocket.Common
{
    ...
    {
        private static void AddNotification(ApplicationDbContext dbc, Notification notification)
        {
            dbc.Notifications.Add(notification);
            dbc.SaveChanges();
        }

        public static void AddExpenseNotification(ApplicationDbContext db, Expense exp, bool AddExpense, string generatedBy)
        {
        ...
        private static void AddExpenseNotification(ApplicationDbContext db, Expense exp, bool AddExpense, string generatedBy, List<string> lstUsers, SharedResourceType rtype)
        {
        ...
        internal static void AddEventShareNotification(int eventID, string eventName, string friendID, ApplicationDbContext db, string generatedBy, string genUserName)
        {
            Notification notif = new Notification();
        ...
        internal static void RemoveEventShareNotification(int eventID, string eventName, string friendID, ApplicationDbContext db, string generatedBy, string genUserName)
        {
            Notification notif = new Notification();
            notif.UserID = friendID;
            notif.GeneratedBy = generatedBy;
        ...
        internal static void AddFriendNotification(string friendID, ApplicationDbContext db, string UserID, string UserName)
        {
            Notification notif = new Notification();
            notif.UserID = friendID;
            notif.GeneratedBy = UserID;
            notif.NotificationDate = DateTime.Now;
            notif.Title = "Friend Request";
            notif.Text = string.Format("'{0}' sent you a friend request.", UserName);
            notif.URL = string.Format("/Friend/Index");
            notif.MobileURL = string.Format("Pages/FriendList.html");
            db.Notifications.Add(notif);
        }
        internal static void AddExpenseCommentNotification(string comment, int ExpenseID, ApplicationDbContext db, string UserID, string UserName)
        {
            ...
            if (exp.EventID.HasValue)
            {
                ...
                foreach (var frUserID in lstUsers)
                {
                    if (frUserID != UserID) // don't generate a notification for myself
                    {
                        Notification notif = new Notification();
                        notif.UserID = frUserID;
                        notif.GeneratedBy = UserID;
        ...
        internal static bool AddExpenseDelNotification(ApplicationDbContext db,string UserID, Expense exp)
        {
            if (exp.EventID.HasValue)
            {
                Event evt = exp.Event;
        ...
        internal static void AddAccountShareNotification(int accountID, string accountName, string friendID, ApplicationDbContext db, string generatedBy, string genUserName)
        {
            Notification notif = new Notification();
            notif.UserID = friendID;
            notif.GeneratedBy = generatedBy;
        ...
        internal static void RemoveAccountShareNotification(int accountid, string accName, string FriendID, ApplicationDbContext db, string generatedBy, string genUserName)
        {
            Notification notif = new Notification();
            notif.UserID = FriendID;
            notif.GeneratedBy = generatedBy;
        ...
        internal static void AddCategoryShareNotification(int categoryID, string categoryName, string friendID, ApplicationDbContext db, string generatedBy, string genUserName)
        {
            Notification notif = new Notification();
            notif.UserID = friendID;
            notif.GeneratedBy = generatedBy;
        ...
        internal static void RemoveCategoryShareNotification(int categoryID, string categoryName, string FriendID, ApplicationDbContext db, string generatedBy, string genUserName)
        {
            Notification notif = new Notification();
            notif.UserID = FriendID;
            notif.GeneratedBy = generatedBy;

...
{
    [Authorize]
    public class TargetController : ApplicationController
    {
        
        public TargetController(ApplicationDbContext
        ...
        {
            var targets = ToTargetResult( db.Targets.Where(t => t.UserID == UserID && t.TargetID == targetid).ToList());
        ...
        {
            var targets = db.Targets.Where(t => t.UserID == UserID);
        ...
        {
            var targets = db.Targets.Where(t => t.UserID == UserID && t.Status == Common.TargetStatus.Active && t.TargetDate >= DateTime.Today);
        ...
        private JsonResult GetList(string sidx, string sord, int page, int rows, ResultType rt)
        {
                ...
                var targets = ToTargetResult(db.Targets.Where(t => t.UserID == UserID).ToList());
                        ...
                        if (rt == ResultType.Web)
	{
        ...
        private T Edit<T>(int TargetID, string Name, double TargetAmount,double InitialAmount, DateTime TargetDate, TargetStatus Status, YesNoOptions Budgeted, RepeatPattern BudgetDuration, double BudgetAmount)where T:JsonResult
        {
                ...
                {
                    target = db.Targets.Where(t=>t.UserID == UserID && t.TargetID == TargetID).FirstOrDefault();
                    if(target == null)
        ...
        public string JTargets()
        {
            string selectStr = "<option value=''></option>";
                ...
                var targets = db.Targets.Where(t=>t.UserID == UserID && t.Status == Common.TargetStatus.Active);
        ...
        private T Delete<T>(int TargetID) where T : JsonResult
        {
                try
                {
Target target = db.Targets.Where(t => t.UserID == UserID && t.TargetID == TargetID).FirstOrDefault();
                if (target != null)
                {

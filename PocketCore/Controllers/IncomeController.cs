...
{
    [Authorize]
    public class IncomeController : ApplicationController
    {
        public IncomeController(ApplicationDbContext
        ...
        {
            return View(db.Users.Find(UserID).Income);
        }
        public JsonResult MRecord(int IncomeID)
        {
            

                var incomes = db.Income.Where(inc => inc.UserID == UserID && inc.IncomeID == IncomeID).ToList();
       ...
       public JsonResult JList(string sidx, string sord, int page, int rows)
        {
            DateTime IncomeFrom = Util.ToDateTime(Request.Form["IncomeFrom"], new DateTime(DateTime.Now.Year, DateTime.Now.Month, 1));
            DateTime IncomeTo = Util.ToDateTime(Request.Form["IncomeTo"], new DateTime(DateTime.Now.Year, DateTime.Now.Month, 1));
        ...
        private JsonResult GetList(string sidx, string sord, int page, int rows, DateTime IncomeFrom, DateTime IncomeTo, ResultType rt)
        {
                ...
                var incomes = db.Users.Find(UserID).Income.Where(inc => inc.IncomeDate >= IncomeFrom && inc.IncomeDate <= IncomeTo).ToList();
                   ...
                   else
	{
        ...
        [HttpPost]
        public JsonResult MEdit([Bind( "IncomeID,AccountID,SourceID,Amount,IncomeDate,Description,Repeat")] int IncomeID, int AccountID, int SourceID,
        ...
        [HttpPost]
        public JsonResult JEdit([Bind( "IncomeID,AccountID,SourceID,Amount,IncomeDate,Description,Repeat")] int IncomeID, int AccountID, int SourceID,
                                                                            double Amount, DateTime IncomeDate, string Description, int Repeat)
        {
            return Edit<JsonResult>(IncomeID, AccountID, SourceID, Amount, IncomeDate, Description, Repeat);
        }

        private T Edit<T>([Bind( "IncomeID,AccountID,SourceID,Amount,IncomeDate,Description,Repeat")] int IncomeID, int AccountID, int SourceID,
                                                                            double Amount, DateTime IncomeDate, string Description, int Repeat)where T:JsonResult
        {
            if (ModelState.IsValid)
            {
                ...
                Income income = null;
                if(IncomeID > 0)
                 income = db.Income.Where(inc => inc.UserID == UserID && inc.IncomeID == IncomeID).FirstOrDefault();
                else
                {
                    income = new Income();
        ...
        private T Delete<T>(int IncomeID) where T : JsonResult
        {
                try
                {
                    Income income = db.Income.Where(inc => inc.UserID == UserID && inc.IncomeID == IncomeID).FirstOrDefault();
                    if (income != null)
                    {
...
return Repository.DelFailure<T>();
        ...
        private bool ValidateIncome(Income income)
        {
            ...
            validated = validated && Global.geAllUserAccounts(db, UserID).Where(acc => acc.AccountID == income.AccountID).FirstOrDefault() != null;

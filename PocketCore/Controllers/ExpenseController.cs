...
{
    [Authorize]
    public class ExpenseController : ApplicationController
    {
        public ExpenseController(ApplicationDbContext
        ...
        {
            //var expenses = db.Users.Find(UserID).Expenses;// db.Expenses.Include(e => e.Payee);
        ...
        public IActionResult MExpenseByID(int ExpenseID)
        {
            var ex = Repository.GetExpenseByID(db,UserID, ExpenseID);
            if (ex == null)
            {
                return BadRequest();
                ...
                new
            {
                ...
                Editable = ex.UserID.ToString() == UserID,
        ...
        public JsonResult MList(int? page, int? rows, DateTime? ExpenseFrom, DateTime? ExpenseTo, bool? AllUsers)
        {
            ...
            if (AllUsers.HasValue && AllUsers.Value)
                ...
                expenses = db.Expenses.Where(exp => exp.UserID.ToString() == UserID);
        ...
        public JsonResult JList(string sidx, string sord, int page, int rows)
        {
                ...
                DateTime expenseFrom = Util.ToDateTime(Request.Form["ExpenseFrom"], new DateTime(DateTime.Now.Year, DateTime.Now.Month, 1));
                DateTime expenseTo = Util.ToDateTime(Request.Form["ExpenseTo"], new DateTime(DateTime.Now.Year, DateTime.Now.Month, 1));
                
                var expenses = db.Users.Find(UserID).Expenses.Where(ex=> ex.ExpenseDate >= expenseFrom && ex.ExpenseDate <= expenseTo).ToList();
        ...
        [HttpPost]
        public JsonResult MEdit([Bind( "ExpenseID,ExpenseDate,SubcategoryID,AccountID,Amount,Description,PayeeID,VendorID,EventID,Repeat")] 
        ...
        [HttpPost]
        public JsonResult JEdit([Bind( "ExpenseID,ExpenseDate,SubcategoryID,AccountID,Amount,Description,PayeeID,VendorID,EventID,Repeat")] 
            int ExpenseID, DateTime ExpenseDate, int SubcategoryID, int AccountID, double Amount, string Description, int? PayeeID, int? VendorID,
            int? EventID, int Repeat)
        {
            return Edit<JsonResult>(ExpenseID, ExpenseDate, SubcategoryID, AccountID, Amount, Description, PayeeID, VendorID, EventID, Repeat);
            ...
            int? EventID, int Repeat) where T:JsonResult
        {
            if ( ModelState.IsValid)
            {
                ...
                if(ExpenseID > 0)
                {
                    exp = db.Expenses.Where(ex => ex.ExpenseID == ExpenseID && ex.UserID == UserID).FirstOrDefault();
                    if (exp == null)
                        return Global.BadRequest<T>();
                    if (exp.EventID.HasValue && exp.EventID != EventID)
                    {
                        NotificationManager.AddExpenseDelNotification(db,UserID, exp);
                    }
                }
                else
                {
                ...
                if(ValidateExpense(exp))
                {
                    ...
                    else
                    {
                        ...
                        db.SaveChanges();
                        NotificationManager.AddExpenseNotification(db, exp, add, UserID);
                        db.SaveChanges();
        ...
        private T Delete<T>(int ExpenseID) where T : JsonResult
        {
                ...
                try
                {
                    Expense expense = db.Expenses.Where(ex => ex.UserID == UserID && ex.ExpenseID == ExpenseID).FirstOrDefault();
                    if (expense != null)
                    {
...
return Repository.DelFailure<T>();
        ...
        public JsonResult JEventExpenses(string sidx, string sord, int page, int rows)
        {
                ...
                var expenses = db.Expenses.Where(ex => ex.UserID == UserID && ex.EventID == eventID).ToList();
        ...
        public JsonResult MExpenseComments(int expenseID)
        {
                ...
                if (ei != null && Util.HasExpenseAccess(ei, UserID))
                {
        ...
        {
            int expenseID = Util.ParseInt(Request.Form["ExpenseID"]);
        ...
        private JsonResult ExpenseComments(string sidx, string sord, int page, int rows, int expenseID )
        {
            ...
            return Json(new StatusCodeResult(StatusCodes.Status400BadRequest));
        }
        ...
        [ValidateAntiForgeryToken]
        [HttpPost]
        public JsonResult jcommentedit([Bind( "Comment,ExpenseID")] string Comment, int ExpenseID)
        {
            return EditComment<JsonResult>(Comment, ExpenseID);
        }
        ...
        private T EditComment<T>(string Comment, int ExpenseID) where T:JsonResult
        {
            if (ModelState.IsValid)
            {
                ...
                if (ei != null && Util.HasExpenseAccess(ei, UserID))
                {
                    ...
                    db.SaveChanges();
                    NotificationManager.AddExpenseCommentNotification(Comment, ec.ExpenseID, db, UserID, UserName);
        ...
        private bool ValidateExpense(Expense expense)
        {
            ...
            if (expense.ExpenseID != 0 && u.Expenses.Where(e => e.ExpenseID == expense.ExpenseID).SingleOrDefault() == null)
                return false;
            if (Common.Global.geAllUserAccounts(db, UserID).SingleOrDefault(a => a.AccountID == expense.AccountID) == null)

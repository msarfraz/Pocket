...
namespace Pocket.Controllers
{
    ...
    {
        public EventController(ApplicationDbContext
        ...
        {
            Event ev = db.Users.Find(UserID).Events.Find(e => e.EventID == id);
            if (ev == null)
            {
                return NotFound();
            }
        ...
        {
            int id = Util.ParseInt( Request.Form["EventID"]);
            if (id == 0)
            {
                return BadRequest();
            }
            ...
            if (ev == null)
            {
                ...
                EventFriends ef = new EventFriends(db, UserID, id);
                ...
                ef.EFriends.ForEach(u=>u.Type = UserType.Application); // used the usertype to flag the user as event user already shared

                ef.Friends.ForEach(u => u.Type = UserType.FB);// used the usertype to flag the user as event user already shared
        ...
        public ActionResult ShareConfirmed(int EventID, string UserID)
        {
                ...
                Event ev = db.Users.Find(UserID).Events.Find(e => e.EventID == EventID);
                ...
                if (ev != null && fr != null)
                {
                    ...
                    db.SaveChanges();
                    NotificationManager.AddEventShareNotification(ev.EventID, ev.Name, UserID, db, UserID,UserName);
        ...
        {
            var events = db.Events.Where(ev => ev.UserID == UserID && ev.EventID == EventID).ToList();
            if(events.Count < 1)
                events = db.EventUsers.Where(eu => eu.UserID == UserID && eu.EventID == EventID).Select(eu => eu.Event).ToList();
            if (events.Count < 1)
            {
                return Global.BadRequest<JsonResult>();
            }
            ...
            return Util.Package<JsonResult>(events.Select(ev => new {
                ...
                Editable = ev.UserID == UserID
            }));
        }
        public JsonResult MList(int? page, int? rows)
        {
           
                var mevents = db.Events.Where(ev => ev.UserID == UserID).ToList();
                ...
                return Util.CreateJsonResponse<Event>("EventDate", "desc", page, rows, events, ResultType.Mobile, (Func<IEnumerable<Event>, Array>)delegate(IEnumerable<Event> rd)
                {
                    ...
                    return (
                        ...
                        select new
                        {
                            ...
                            EventStatus = ev.EventStatus.String(),
                            Name = ev.Name,
                            UserName = ev.UserID == UserID? "":ev.User.UserName,
                            Editable = ev.UserID == UserID
        ...
        public JsonResult MUpcomingList()
        {
            if (true)
            {
                var mevents = db.Events.Where(ev => ev.UserID == UserID && ev.EventDate >= DateTime.Today).ToList();
                var fevents = db.EventUsers.Where(eu => eu.UserID == UserID && eu.Event.EventDate >= DateTime.Today).Select(eu => eu.Event).ToList();
        ...
        private JsonResult GetList(string sidx, string sord, int page, int rows, ResultType rt)
        {
                ...
                var events = db.Users.Find(UserID).Events;
                foreach (var ev in events)
                {
                    ev.Amount = db.Expenses.Where(e => e.EventID == ev.EventID).Select(ex => ex.Amount).DefaultIfEmpty(0).Sum();
                }
        ...
        private JsonResult GetOtherList(string sidx, string sord, int page, int rows, ResultType rt)
        {
                ...
                var events = db.Users.Find(UserID).OtherEvents.Select(ev=>ev.Event);
                foreach (var ev in events)
                {
                    ev.Amount = db.Expenses.Where(e => e.EventID == ev.EventID).Select(ex => ex.Amount).DefaultIfEmpty(0).Sum();
                }
        ...
        private T Edit<T>(int EventID, string Name, DateTime EventDate, int BudgetDuration, double BudgetAmount, YesNoOptions Budgeted) where T : JsonResult
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    ev = db.Users.Find(UserID).Events.Find(t => t.EventID == EventID);
                    if (ev == null)
        ...
        private T Delete<T>(int EventID) where T : JsonResult
        {
                ...
                try
                {
                    Event ev = db.Events.Where(ex => ex.UserID == UserID && ex.EventID == EventID).FirstOrDefault();
                    if (ev != null)
                    {
                        db.Events.Remove(ev);
                        db.SaveChanges();
        ...
        public JsonResult MEvents()
        {
                ...
                var mevents = db.Events.Where(ev => ev.UserID == UserID && ev.EventStatus == EventStatus.Active).ToList();
                var fevents = db.EventUsers.Where(eu => eu.UserID == UserID && eu.Event.EventStatus == EventStatus.Active).Select(eu => eu.Event).ToList();
                var events = mevents.Union(fevents);

                foreach (var ev in events)
                {
                    groups.AddGroupDetails(ev.UserID == UserID ? "" : ev.User.UserName, new MutableTuple<int, string>(ev.EventID, ev.Name));
                }

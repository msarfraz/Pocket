...
{
    [Authorize]
    public class ChartController : ApplicationController
    {
        public ChartController(ApplicationDbContext
        ...
        {
            var cats = Repository.GetCategoryBudgetAmount(db,UserID, Month, Year, false);
            if (includeEvents)
            {
        ...
        public ActionResult SubcatChart()
        {
            ...
            var cats = db.Categories.Where(c=>c.UserID == UserID && c.Display == DisplaySetting.Yes);
            var ucats = db.CategoryUsers.Where(uc => uc.UserID == UserID && uc.Display == DisplaySetting.Yes).Select(uc => uc.Category);
...
//            var labels = new object[] { new object[] { "Subcategory", "Expense", "Budget" } };
  //          var data = labels.Union(Data);
        ...
        private List<SubcategoryBudgetAmount> getSubCategoryBudgetAndAmount(int Month, int Year, int categoryID)
        {
            ...
            var cats = Repository.GetCategoryBudgetAmount(db,UserID, Month, Year, false, categoryID);
            if (cats.Count > 0)
        ...
        private List<CategoryBudgetAmount> getEventBudgetAmount(ApplicationDbContext db, int Month, int Year)
        {
            // events of the month
            var monthEvents = db.Events.Where( ev => ev.UserID == UserID && ev.EventDate.Month == Month && ev.EventDate.Year == Year);
            // expenses occurred in the month for any events
            var allevents = db.Events.Where(ev => ev.UserID == UserID).Select(ev=>ev.EventID).Union(db.EventUsers.Where(evu => evu.UserID == UserID).Select(evu => evu.Event.EventID)).ToArray();
        ...
        public JsonResult MBudgetChartData(string yearMonth)
        {
            ...
            var budget = Global.getMonthBudget(db,UserID, dt.Month, dt.Year, false);

            var expense = Global.getMonthExpense(db,UserID, dt.Month, dt.Year);
      ...
      /*          al.Add(days.ToArray());
        ...
        List<HomeChartPoint> getIncomeSeries(ApplicationDbContext db, int Month, int Year)
        {
            var myincome = from inc in db.Income
                           where inc.UserID == UserID && inc.IncomeDate.Month == Month && inc.IncomeDate.Year == Year
        ...
        List<HomeChartPoint> getExpenseSeries(ApplicationDbContext db, int Month, int Year)
        {
            var mcats = db.Categories.Where(cat => cat.UserID == UserID);
            var fcats = db.CategoryUsers.Where(cu => cu.UserID == UserID).Select(cu=>cu.Category);
            var allcats = mcats.Union(fcats);

            var mevents = db.Events.Where(ev => ev.UserID == UserID && ev.Budgeted == YesNoOptions.Yes);
            var fevents = db.EventUsers.Where(eu => eu.UserID == UserID && eu.Event.Budgeted == YesNoOptions.Yes).Select(eu=>eu.Event);
            ...
            var cexpense = from exp in db.Expenses
                            ...
                            (!exp.EventID.HasValue || (exp.Event.Budgeted == YesNoOptions.No && exp.UserID == UserID))
                                select exp;
        ...
        private List<HomeChartPoint> getBudgetSeries(ApplicationDbContext db, int Month, int Year, bool ConstantBudget)
        {
            double mbudget = Pocket.Common.Global.getMonthBudget(db, UserID, Month, Year, ConstantBudget);
        ...
        private List<HomeChartPoint> getEventSeries(ApplicationDbContext db, int Month, int Year)
        {
            var events = db.Users.Find(UserID).Events.Where(ev => ev.EventDate.Month == Month && ev.EventDate.Year == Year).OrderBy(ev => ev.EventDate);
        ...
        private List<HomeChartPoint> getTargetSeries(ApplicationDbContext db, int Month, int Year)
        {
            var targets = db.Targets.Where(t => t.UserID == UserID && t.TargetDate.Month == Month && t.TargetDate.Year == Year).OrderBy(ev => ev.TargetDate);
            List<HomeChartPoint> series = new List<HomeChartPoint>();
        ...
        public ActionResult TargetChart()
        {
            ...
            var mbudget = Pocket.Common.Global.getMonthBudget(db,UserID, DateTime.Now.Month, DateTime.Now.Year, false);
        ...
        public T MoneyFlowChartData<T>(int Month, int Year) where T : JsonResult
        {
            ...
            nodes.Add(parent);
            var accounts = Pocket.Common.Global.geAllUserAccounts(db, UserID).ToList();

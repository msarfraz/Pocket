...
{
    [Authorize]
    public class PayeeController : ApplicationController
    {
        public PayeeController(ApplicationDbContext
        ...
        {
            var payees = db.Users.Find(UserID).Payees;
            return View(payees.ToList());
        }
        [HttpPost]
        public JsonResult MEdit([Bind( "PayeeID,Name")] int PayeeID, string Name)
        {
        ...
        [HttpPost]
        public JsonResult JEdit([Bind( "PayeeID,Name")] int PayeeID, string Name)
        {
            return Edit<JsonResult>(PayeeID, Name);
        }
       
        private T Edit<T>([Bind( "PayeeID,Name")] int PayeeID, string Name) where T:JsonResult
        {
            if (ModelState.IsValid)
            {
                Payee payee = db.Payees.Where(p => p.PayeeID == PayeeID && p.UserID == UserID).FirstOrDefault();
                if (payee == null) //add
                {
                    payee = new Payee();
                payee.UserID = UserID;
        ...
        private T Delete<T>(int PayeeID) where T : JsonResult
        {
                try
                {
Payee payee = db.Payees.Where(p => p.UserID == UserID && p.PayeeID == PayeeID).FirstOrDefault();
                if (payee != null)
                {

...
namespace Pocket.Common
{
    public static class Repository
    {
        ...
        public static IQueryable<Expense> GetAllAccessibleExpenses(QDbContext db)
        {
       ...
       public static IQueryable<Expense> GetEventExpenses(QDbContext db, int EventID)
        {
            return GetExpenses(db, null, null, null,null, EventID, null, null, null, true);
        }
        public static Expense GetExpenseByID(QDbContext db, int expenseID)
       {
           var expenses = Repository.GetAllAccessibleExpenses(db);
           return expenses.Where(ex => ex.ExpenseID == expenseID).SingleOrDefault();

       }
        public static IQueryable<Expense> GetExpenses(QDbContext db, int? AccountID, int? SubcatID, int? PayeeID, int? VendorID, int? EventID, int? ExpenseID, DateTime? FromDate, DateTime? ToDate, bool AllUsers)
        {
        ...
        /// <returns></returns>
        internal static List<CategoryBudgetAmount> GetCategoryBudget(QDbContext db)
        {
            int Month = DateTime.Today.Month;
            int Year = DateTime.Today.Year;
            var cats = db.Categories.Where(c => c.UserID == State.UserID).ToList(); // db.Users.Find(State.UserID).Categories;
            return GetMonthlyCatBudget(db, cats);
        }
        /// <summary>
        /// category expenses w.r.t month
        /// </summary>
        ...
        /// <param name="CategoryID"></param>
        /// <returns></returns>
        internal static List<CategoryBudgetAmount> GetSharedCategoryBudget(QDbContext db)
        {
            int Month = DateTime.Today.Month;
            int Year = DateTime.Today.Year;

            var cats = db.CategoryUsers.Where(c => c.UserID == State.UserID).Select(cu=>cu.Category).ToList(); // db.Users.Find(State.UserID).Categories;
            return GetMonthlyCatBudget(db, cats);
        }
        internal static List<CategoryBudgetAmount> GetAllCategoryBudget(QDbContext db)
        {
            int Month = DateTime.Today.Month;
            int Year = DateTime.Today.Year;
            var cats = db.Categories.Where(c => c.UserID == State.UserID).ToList();
            var ucats = db.CategoryUsers.Where(c => c.UserID == State.UserID).Select(cu => cu.Category).ToList(); // db.Users.Find(State.UserID).Categories;
            return GetMonthlyCatBudget(db, cats.Union(ucats).ToList());
        }
        internal static List<CategoryBudgetAmount> GetMonthlyCatBudget(QDbContext db, List<Category> cats)
        {
            ...
            foreach (var cat in cats.ToList())
            {
                ...
                {
                    double sbudget = GetSubcatMonthlyBudget(db, scat, Month, Year, true);
        ...
        /// <param name="CategoryID"></param>
        /// <returns></returns>
        internal static List<CategoryBudgetAmount> GetCategoryBudgetAmount(QDbContext db, int Month, int Year, bool ConstantBudget, int? CategoryID = null)
        {
            List<CategoryBudgetAmount> catsba = new List<CategoryBudgetAmount>();
            ...
            foreach (var cat in cats.ToList())
            {
                ...
                foreach (var scat in cat.Subcategories)
                {
                    double sbudget = GetSubcatMonthlyBudget(db, scat, Month, Year, ConstantBudget);
        ...
        /// <returns></returns>
        internal static IQueryable<Expense> GetSubcatExpenses(QDbContext db, int Month, int Year, int SubcategoryID)
        {
        ...
        /// <param name="Year"></param>
        /// <returns></returns>
        internal static double GetSubcatMonthlyBudget(QDbContext db, Subcategory subcat, int Month, int Year, bool constantBudget)
        {
            double budget = 0;
                ...
                case RepeatPattern.Bi_Monthly:
                    ...
                    else
                    {
                        if (Month % 2 == 1) // first month
                            ...
                            budget = subcat.Budget.BudgetAmount - GetSubcatExpense(db, subcat, dt, false);
                    }
                ...
                case RepeatPattern.Quarterly:
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            budget = budget - GetSubcatExpense(db, subcat, dt, false);
                        }
        ...
        /// <returns></returns>
        internal static double GetSubcatExpense(QDbContext db, Subcategory subcat, DateTime dtTo, bool dateInclusive)
        {
            IQueryable<Expense> expenses = null;
            ...
            switch (subcat.Budget.BudgetDuration)
            {
                case RepeatPattern.None:
                    break;
                case RepeatPattern.Daily:
                    expenses = db.Expenses.Where(exp => exp.UserID == State.UserID && exp.SubcategoryID == subcat.SubcategoryID && (exp.ExpenseDate < dtTo || (dateInclusive && exp.ExpenseDate == dtTo)));
                    break;
                case RepeatPattern.Alternate_Days:
                    var day = dtTo.Day;
                    ...
                    expenses = db.Expenses.Where(exp => exp.UserID == State.UserID && exp.SubcategoryID == subcat.SubcategoryID && exp.ExpenseDate >= dt && (exp.ExpenseDate < dtTo || (dateInclusive && exp.ExpenseDate == dtTo)));
                    break;
                case RepeatPattern.Weekly:

...
namespace Pocket.Controllers
{
    ...
    public class SavingController : Controller
    {
        ...
        {
            var savings = db.Savings.Where(s => s.UserID == State.UserID && s.SavingID == SavingID).ToList();
        ...
        {
                var savings = db.Savings.Where(s => s.UserID == State.UserID);
        ...
        private T Edit<T>(int SavingID, int TargetID, int AccountID, double Amount, DateTime SavingDate) where T : JsonResult
        {
                ...
                {
                    saving = new Saving();
                    saving.UserID = State.UserID;
                    saving.CreatedDate = DateTime.Now;
                }
                else
                {
                    saving = db.Savings.Where(s => s.UserID == State.UserID && s.SavingID == SavingID).FirstOrDefault();
                    if (saving == null)
                    {
        ...
        private T Delete<T>(int SavingID) where T : JsonResult
        {
                try
                {
Saving saving = db.Savings.Where(s => s.UserID == State.UserID && s.SavingID == SavingID).FirstOrDefault();
                if (saving != null)
                {

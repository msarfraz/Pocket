...
namespace Pocket.Controllers
{
    ...
    {
        private QDbContext qdb = new QDbContext();
        ...
        {
                var schedules = qdb.Schedules.Where(sch=>sch.Status != ScheduleStatus.Complete).ToList();
                var evSchedules = qdb.Events.Where(ev => ev.EventStatus == EventStatus.Active && ev.EventDate <= DateTime.Today).ToList().Select(
        ...
        {
                var schedules = qdb.ScheduleJobs.AsEnumerable();
        ...
        public JsonResult ProcessSchedules()
        {
            ...
            return ProcessSchedules(qdb);

        }

        internal JsonResult ProcessSchedules(QDbContext db)
        {
           ...
           // }
            ...
            db.Entry(sjob).State = System.Data.Entity.EntityState.Modified;
            try
            {

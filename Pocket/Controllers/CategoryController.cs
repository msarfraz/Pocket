...
namespace Pocket.Controllers
{
    ...
    public class CategoryController : Controller
    {
        ...
        public JsonResult MCategories()
        {
                ...
                var mcategories = db.Categories.Where(cat => cat.UserID == State.UserID).ToList();
                var fcategories = db.CategoryUsers.Where(ucat => ucat.UserID == State.UserID).Select(uc => uc.Category).ToList();
                ...
                foreach (var cat in categories)
                {
                    ...
                    groups.AddGroupDetails(cat.UserID == State.UserID ? "" : cat.User.UserName, new MutableTuple<int, string, List<MutableTuple<int, string>>>(cat.CategoryID, cat.Name, subcats));
                }
        ...
        public JsonResult MCategoryByID(int CategoryID)
        {
                ...
                var categories = Repository.GetAllCategoryBudget(db).Where(cba=>cba.CategoryID == CategoryID).ToList();
        ...
        public JsonResult MList()
        {
                ...
                var categories = Repository.GetAllCategoryBudget(db);
        ...
        public JsonResult JList(string sidx, string sord, int page, int rows)
        {
                ...
                var categories = Repository.GetCategoryBudget(db);
        ...
        public JsonResult JSharedList(string sidx, string sord, int page, int rows)
        {
                ...
                var categories = Repository.GetSharedCategoryBudget(db);
                //var categories = db.Categories.Where(cat => cat.UserID == State.UserID).AsNoTracking();
        ...
        [HttpPost]
        public JsonResult JSharedEdit([Bind(Include = "CategoryID,Display")] int CategoryID,  DisplaySetting Display)
        {
            if (ModelState.IsValid)
            {
                var catu = db.CategoryUsers.Where(cu => cu.UserID == State.UserID && cu.CategoryID == CategoryID).SingleOrDefault();
                if (catu != null)
                {
        ...
        [ValidateAntiForgeryToken]
        [HttpPost]
        public JsonResult JEdit([Bind(Include = "CategoryID,Name")] int CategoryID, string Name)
        {
            return Edit<JsonResult>(CategoryID, Name);
        }
        ...
        private T Edit<T>(int CategoryID, string Name) where T:JsonResult
        {
            if (ModelState.IsValid)
            {
                Category cat = new Category { UserID = State.UserID };
                ...
                {
                    cat = db.Categories.Where(c => c.UserID == State.UserID && c.CategoryID == CategoryID).SingleOrDefault();
                    if (cat == null)
                    {
        ...
        private T Delete<T>(int CategoryID) where T : JsonResult
        {
                ...
                try
                {
                    Category cat = db.Categories.Where(ex => ex.UserID == State.UserID && ex.CategoryID == CategoryID).FirstOrDefault();
                    if (cat != null)
                    {
...
return Repository.DelFailure<T>();
        ...
        public string JCategories()
        {
            ...
            DateTime currMonth = DateTime.Today;
                ...
                var categories = db.Users.Find(State.UserID).Categories.Where(cat=>cat.Display == DisplaySetting.Yes).ToList();
                foreach (var category in categories)
                {
                    var sc = string.Empty;
                    foreach (var subcat in category.Subcategories)
                    {
                        double amount = Repository.GetSubcatExpense(db, subcat, currMonth, true);
                        string color = "black";
                ...
                var sharedcats = db.CategoryUsers.Where(cu => cu.UserID == State.UserID && cu.Display == DisplaySetting.Yes).Select(cu => cu.Category).ToList();
        ...
        public JsonResult JCategoryValues()
        {
            ...
            var cats = db.Users.Find(State.UserID).Categories.ToList();
            ...
            var sharedcats = db.CategoryUsers.Where(cu => cu.UserID == State.UserID).Select(cu => cu.Category).ToList();
